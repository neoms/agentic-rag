...
import { ChevronDown, FileText, ExternalLink } from 'lucide-vue-next'
...
<template>
  <div class="px-3">
    ...
    >
      ...
      <span>参考 {{ sources.length }} 个来源文档</span>
      <ChevronDown
        :class="[
    ...
    >
      <div
        ...
        :key="idx"
        class="bg-slate-800/60 border border-slate-700/50 rounded-lg p-3"
      >
        <div class="flex items-center justify-between mb-1.5">
          <div class="flex items-center gap-2">
            <span class="text-[10px] font-medium text-slate-500 bg-slate-700/50 px-1.5 py-0.5 rounded">
              #{{ idx + 1 }}
            </span>
            <span
              v-if="source.score !== null"
              :class="['text-[10px] font-medium', getScoreColor(source.score)]"
            >
        ...
        <div
          v-if="Object.keys(source.metadata).length > 0"
